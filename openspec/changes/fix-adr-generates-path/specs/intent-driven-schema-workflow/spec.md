...
### User Story: Intent-driven schema persists durable decisions as top-level ADRs
As a schema user, I want the `intent-driven` ADR artifact to report its generated files under the repository-level ADR folder, so that OpenSpec does not consider ADR creation complete based on unrelated change files such as `proposal.md`.
...
#### Acceptance Criteria
- **Given** implementation changes files under `openspec/schemas/intent-driven/`
- **When** the implementation is complete
- **Then** `openspec schema review intent-driven` MUST pass before the change is considered complete
