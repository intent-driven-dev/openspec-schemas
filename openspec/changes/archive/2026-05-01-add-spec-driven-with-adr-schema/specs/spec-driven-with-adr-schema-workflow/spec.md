...
The `spec-driven-with-adr` schema SHALL expose the artifacts `proposal`, `specs`, `design`, `adr`, and `tasks`, and SHALL require those artifacts to be completed in dependency order before apply readiness.
